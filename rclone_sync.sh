...
if [ -f "/root/rclone_sync.sh" ]; then
    local_sha=$(sha256sum /root/rclone_sync.sh | sed 's/\(^\w*\).*/\1/')
    remote_sha=$(curl -s https://raw.githubusercontent.com/ypq123456789/rclone_sync/main/rclone_sync.sh | sha256sum | sed 's/\(^\w*\).*/\1/')
